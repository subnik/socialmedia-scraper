...
module.exports.searchChannel = async (q, options = {}) => {
  ...
  const res = await fetch(['https://www.youtube.com/results', `?search_query=${q}`, `&sp=${filter}`].join(''), {
    method: 'GET',
    credentials: 'include',
  ...
  try {
    list = json.contents.twoColumnSearchResultsRenderer.primaryContents.sectionListRenderer.contents[0].itemSectionRenderer.contents.map(i => {
...
module.exports.getChannelVideos = async (channelId, options = {}) => {
  ...
  try {
    list = json.contents.twoColumnBrowseResultsRenderer.tabs[1].tabRenderer.content.sectionListRenderer.contents[0].itemSectionRenderer.contents[0].gridRenderer.items.map(i => i && i.gridVideoRenderer && i.gridVideoRenderer.publishedTimeText ? i.gridVideoRenderer.videoId : null).filter(i => i !== null);
  } catch (err) {
    throw new Error('error constructing data');
  }
