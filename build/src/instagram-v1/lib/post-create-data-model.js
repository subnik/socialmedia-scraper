const getErrorOrigin = require('./lib/get-origin');
...
const dataModelCommentsList = json => json.edges.length === 0 ? [] : json.edges.map(dataModelComment); // post caption


const dataModelCaption = json => json.edge_media_to_caption.edges.length === 0 ? '' : json.edge_media_to_caption.edges[0].node.text.toLowerCase(); // post likes data


const dataModelLikesList = json => json.edge_media_preview_like.edges.length === 0 ? [] : json.edge_media_preview_like.edges.map(item => ({
  id: item.node.id,
...
const dataModelSponsorsList = json => json.edge_media_to_sponsor_user.edges.length === 0 ? [] : json.edge_media_to_sponsor_user.edges.map(item => ({
...
const dataModelMentionsList = (text, sponsorsList) => getTextUsernames(text).map(mention => ({
