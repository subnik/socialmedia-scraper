...
const dataModelMentionsList = (text, emails) => {
  ...
  const emailsText = emails.join(' '); // push mention to usernames array if not indexOf an email

  if (mentions.length !== 0) {
    mentions.map(mention => emailsText.indexOf(mention) === -1 ? usernames.push(mention) : null);
  }
...
const postDataModel = post => {
  ...
  const caption = data.edge_media_to_caption.edges.length === 0 ? '' : data.edge_media_to_caption.edges[0].node.text.toLowerCase();
  return {
    id: data.id,
...
const profileDataModel = json => {
  ...
  try {
    ...
    const postsList = media.edges.length === 0 ? [] : media.edges.map(postDataModel); // postsList excluding post younger than 2 day

    const postsListShortened = postsList.filter(post => new Date() - post.timestamp >= 86400000);
    ...
    return {
      ...
      hashtagsPerPost: allPostsHashtagsList.length ? (allPostsHashtagsList.length / postsList.length).toFixed(3) / 1 : 0
    };
  } catch (err) {
